...
3. 刷新当前页面时浏览器会根据当前运行环境内存来决定是从 Memory Cache 还是 从Disk Cache中拿(可以看到下图最后几个文件有时候是从 Memory Cache中拿有时候是从Disk Cache中拿) ![这是我的实验过程](https://jayzangwill.github.io/blog/img/cache/weizhi_1.gif)
...
![request&response](img/167e8739bb344367)
...
![cache-control](img/167e8739bc724fcb)
...
1. 首次请求
   ...
   ![First request](img/167e8739bc1fe97b)
...
1. 当把服务器响应设置为 `Cache-Control: no-cache` 时，我们发现打开页面之后，三种资源都只被请求 **1** 次。
   ...
   ![只请求一次](img/167e8739bc1fe97b)

   

   

   ![只请求一次](https://user-gold-cdn.xitu.io/2018/12/26/167e8739bd65e62e?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
...
2. 当把服务器响应设置为 `Cache-Control: no-store` 时，情况发生了变化，三种资源都被请求了 **2** 次。而图片因为还多一次异步请求，总计 **3** 次。(红框中的都是那一次异步请求)
   ...
   ![请求多次](img/167e8739ed290f92)

   

   

   ![请求多次](img/167e8739ed347f62)
...
2. 第二次访问(无论关闭 TAB 重新打开，还是直接按 F5 刷新)都能看到所有的请求标记为 `from SerciceWorker`。
   ...
   ![Service Worker 服务器](https://user-gold-cdn.xitu.io/2018/12/26/167e873a0a2c1129?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
